...
import { ContractTransactionReceipt, ContractTransactionResponse, Typed } from "ethers";
...
describe("Method: safeMint: ", () => {
  ...
  describe("When one of parameters is incorrect", () => {
    it("When signer not equal operator", async () => {
      ...
      await expect(achievementsContract.connect(user).safeMint(mintParams, notOperatorSignature))
    ...
    it("When user change data", async () => {
      const { achievementsContract, domain, types, mintParams, minter, user } =
        await loadFixture(deployAchievements);
    ...
    it("When try mint with wrong userAddress", async () => {
      const { achievementsContract, domain, types, mintParams, minter, user, operator } =
        await loadFixture(deployAchievements);
  ...
  describe("When all parameters correct with one NFT token", () => {
    let user: HardhatEthersSigner, minter: HardhatEthersSigner;
    ...
    let userNonceBeforeMint: bigint;
    ...
    it("should increase user nonce token", async () => {
      ...
      expect(userNonce).to.be.equal(userNonceBeforeMint + 1n);
    });

    it("should increase totalSupply", async () => {
      const totalSupply = await achievementsContract.totalSupply(Typed.uint(nftId));
  ...
  describe("When all parameters correct with some NFT token", () => {
    ...
    describe("should increase totalSupply:", () => {
      it("should increase totalSupply of NFT with id 1", async () => {
        const totalSupply = await achievementsContract.totalSupply(Typed.uint(1));
        expect(totalSupply).to.be.equal(1);
      });

      it("should increase totalSupply of NFT with id 2", async () => {
        const totalSupply = await achievementsContract.totalSupply(Typed.uint(2));
        expect(totalSupply).to.be.equal(1);
      });

      it("should increase totalSupply of NFT with id 3", async () => {
        const totalSupply = await achievementsContract.totalSupply(Typed.uint(3));
        expect(totalSupply).to.be.equal(1);
      });
    });
